...
if st.session_state.breakpoints and st.session_state.prices:
    ...
    if st.session_state.history:
        ...
        # 绘制散点图
        ax.scatter(people_vals, income_vals, color='blue', alpha=0.5, label='Records')
        ...
        if len(st.session_state.history) > 1:
            ...
            # 绘制折线
            ax.plot(sorted_people, sorted_income, 'r-', linewidth=2, label='Trend')
        ...
        for i, bp in enumerate(st.session_state.breakpoints):
            ...
            ax.text(bp, min(income_vals), f"Seg {i}", 
    ...
    # 设置图表属性 - 完全使用英文
    ax.set_title("Income Change Curve")  # 图表标题改为英文
    ax.set_xlabel("People")  # X轴标签
    ax.set_ylabel("Income")  # Y轴标签
    ax.grid(True)
    ax.legend()
